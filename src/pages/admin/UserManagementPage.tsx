...
export default function UserManagementPage() {
  ...
  const loadUserProjects = async () => {
    try {
      const response = await AdminApiService.getUserProjects(selected.id);
      if (response.success) {
  ...
  const loadUserPayments = async () => {
    try {
      // For now, use a placeholder since we don't have a getUserPayments endpoint
      setUserPayments([]);
    } catch (error) {
      setUserPayments([]);
    }
  ...
  const upgradeSubscription = async (userId: string) => {
    setIsLoading(true);
    try {
      const response = await AdminApiService.updateUserSubscription(userId, {
        plan_type: "pro",
  ...
  const grantTemporaryAccess = async (userId: string, accessType: string, durationDays: number) => {
    setIsLoading(true);
    try {
      const response = await AdminApiService.grantTemporaryAccess(userId, accessType, durationDays);
      if (response.success) {
        setUsers(prev => prev.map(u => u.id === userId ? { ...u, ...response.data } : u));
        toast.success(`${accessType} access granted for ${durationDays} days`);
  ...
  const promoteToAdmin = async (userId: string, adminRole: string) => {
    console.log('Promoting user:', { userId, adminRole });
    setIsLoading(true);
    try {
      const response = await AdminApiService.promoteToAdmin(userId, adminRole);
      if (response.success) {
        setUsers(prev => prev.map(u => u.id === userId ? { ...u, ...response.data } : u));
        toast.success(`User promoted to ${adminRole}`);
        setPromoteUser(null);
      } else {
  ...
  const removeAdminRole = async (userId: string, adminRole: string) => {
    setIsLoading(true);
    try {
      const reponse = await AdminApiService.demoteFromAdmin(userId);
      if (reponse.success) {
        setUsers(prev => prev.map(u => u.id === userId ? { ...u, ...reponse.data } : u));
        toast.success(`Admin role removed for ${adminRole}`);
        setPromoteUser(null);
      } else {
        toast.error(reponse.error || "Failed to remove admin role");
      }
    } catch (error) {
  ...
  const filtered = users?.filter(u => {
    const fullName = u.first_name && u.last_name ? `${u.first_name} ${u.last_name}` : u.first_name || u.last_name || "";
    return fullName.toLowerCase().includes(search.toLowerCase()) || u.email?.toLowerCase().includes(search.toLowerCase());
  }) || [];
  ...
  const getUserAdminRole = (user: any) => {
  if (!user) return null;
  return user.admin_role || null;
};

const getUserRole = (user: any) => {
  const adminRole = getUserAdminRole(user);
  if (adminRole) {
    return adminRoleLabels[adminRole] || adminRole;
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="border rounded-lg overflow-x-auto">
        <Table>
          <TableHeader><TableRow><TableHead>Name</TableHead><TableHead>Email</TableHead><TableHead>Joined</TableHead><TableHead>Workspaces</TableHead><TableHead>Projects</TableHead><TableHead>Plan</TableHead><TableHead>Status</TableHead><TableHead>Role</TableHead><TableHead>Actions</TableHead></TableRow></TableHeader>
          <TableBody>
            {paginated.map((u: any) => {
              const userRole = getUserRole(u);
              const adminRole = getUserAdminRole(u);
              return (
                <TableRow key={u.id}>
                  <TableCell className="font-medium">{u.first_name && u.last_name ? `${u.first_name} ${u.last_name}` : u.first_name || u.last_name || "—"}</TableCell>
                  <TableCell className="text-sm">{u.email}</TableCell>
                  <TableCell className="text-sm text-muted-foreground">{new Date(u.created_at).toLocaleDateString()}</TableCell>
                  <TableCell>{u.workspaces_count || 0}</TableCell>
                  <TableCell>{u.projects_count || 0}</TableCell>
                  ...
                  <TableCell>{adminRole ? <Badge className="bg-accent/10 text-accent text-xs">{adminRoleLabels[adminRole] || adminRole}</Badge> : <Badge variant="outline" className="text-xs">{userRole}</Badge>}</TableCell>
                  <TableCell>
                    <div className="flex gap-1">
                      <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => setSelected(u)} title="View"><Eye className="h-3.5 w-3.5" /></Button>
                      {canPromoteToAdmin && <Button variant="ghost" size="icon" className="h-7 w-7 text-accent" onClick={() => { setPromoteUser(u); setSelectedAdminRole(""); }} title="Promote"><Shield className="h-3.5 w-3.5" /></Button>}
                      ...
                      {canManageUsers && u.user_status !== "suspended" && <Button variant="ghost" size="icon" className="h-7 w-7 text-destructive" onClick={() => {
                        const fullName = u.first_name && u.last_name ? `${u.first_name} ${u.last_name}` : u.first_name || u.last_name || u.email;
                        setConfirmAction({ type: "suspended", userId: u.id, userName: fullName });
                      }} title="Suspend"><UserX className="h-3.5 w-3.5" /></Button>}
                      {canManageUsers && u.user_status === "suspended" && <Button variant="ghost" size="icon" className="h-7 w-7 text-green-600" onClick={() => {
                        const fullName = u.first_name && u.last_name ? `${u.first_name} ${u.last_name}` : u.first_name || u.last_name || u.email;
                        setConfirmAction({ type: "active", userId: u.id, userName: fullName });
                      }} title="Activate"><UserCheck className="h-3.5 w-3.5" /></Button>}
                      {canManageUsers && u.user_status !== "deactivated" && u.user_status !== "suspended" && <Button variant="ghost" size="icon" className="h-7 w-7 text-muted-foreground" onClick={() => {
                        const fullName = u.first_name && u.last_name ? `${u.first_name} ${u.last_name}` : u.first_name || u.last_name || u.email;
                        setConfirmAction({ type: "deactivated", userId: u.id, userName: fullName });
                      }} title="Deactivate"><UserX className="h-3.5 w-3.5" /></Button>}
                    </div>
                  </TableCell>
                </TableRow>
              );
      ...
      <Dialog open={!!selected} onOpenChange={() => setSelected(null)}>
        <DialogContent className="max-w-3xl max-h-[80vh] overflow-auto">
          <DialogHeader><DialogTitle>{selected?.first_name && selected?.last_name ? `${selected.first_name} ${selected.last_name}` : selected?.first_name || selected?.last_name || selected?.email}</DialogTitle></DialogHeader>
          {selected && (
            <Tabs defaultValue="profile">
              <TabsList>
                <TabsTrigger value="profile">Profile</TabsTrigger>
                <TabsTrigger value="projects">Projects ({selected.projects_count || 0})</TabsTrigger>
                <TabsTrigger value="payments">Payments ({selected.payments_count || 0})</TabsTrigger>
              </TabsList>
              <TabsContent value="profile" className="mt-4 space-y-4">
                <div className="grid grid-cols-2 gap-3 text-sm">
                  <div><span className="text-muted-foreground">Email:</span> {selected.email}</div>
                  <div><span className="text-muted-foreground">Name:</span> {selected.first_name && selected.last_name ? `${selected.first_name} ${selected.last_name}` : selected.first_name || selected.last_name || "—"}</div>
                  ...
                  <div><span className="text-muted-foreground">Workspaces:</span> {selected.workspaces_count || 0}</div>
                  <div><span className="text-muted-foreground">Role:</span> {getUserRole(selected)}</div>
                  <div><span className="text-muted-foreground">Admin Role:</span> {getUserAdminRole(selected) ? adminRoleLabels[getUserAdminRole(selected)!] : "—"}</div>
                </div>
                <div className="flex gap-2">
              ...
              <TabsContent value="projects" className="mt-4">
                {userProjects.length === 0 ? <p className="text-sm text-muted-foreground py-4">No projects</p> : (
              ...
              <TabsContent value="payments" className="mt-4">
                {userPayments.length === 0 ? <p className="text-sm text-muted-foreground py-4">No payments</p> : (
                  <Table>
      ...
      <Dialog open={!!promoteUser} onOpenChange={() => { setPromoteUser(null); setSelectedAdminRole(""); }}>
        <DialogContent>
          ...
          <div className="space-y-4 pt-2">
            ...
            <div className="flex gap-2">
              <Button onClick={() => promoteUser && selectedAdminRole && promoteToAdmin(promoteUser.id, selectedAdminRole)} disabled={!selectedAdminRole || isLoading} className="flex-1">
                <Crown className="h-4 w-4 mr-1" /> Promote
              </Button>
              {getUserAdminRole(promoteUser) && <Button variant="destructive" onClick={() => promoteUser && removeAdminRole(promoteUser.id, getUserAdminRole(promoteUser)!)} disabled={isLoading}>Remove Role</Button>}
            </div>
          </div>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={!!grantAccessUser} onOpenChange={() => setGrantAccessUser(null)}>
        <DialogContent>
          ...
          <div className="space-y-4 pt-2">
            ...
            <Button onClick={() => grantAccessUser && grantTemporaryAccess(grantAccessUser.id, "free", parseInt(grantDuration))} disabled={isLoading} className="w-full">
